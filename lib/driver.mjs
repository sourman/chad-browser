...
async function handle(conn, line) {
  ...
  const bodyTimeoutMs = Number.isFinite(req.timeout) && req.timeout > 0
    ...
    : 120_000;
  const evalTimeout = makeEvalTimeout(bodyTimeoutMs);
  ...
  const boundWaitForReady = (opts) => waitForReady(cdp, opts, evalTimeout);
  const boundNavigate = (url, opts) => navigate(url, opts, evalTimeout);
  const boundWaitForNavigation = (opts, trigger) => waitForNavigation(opts, trigger, evalTimeout);
  try {
    const timeoutPromise = evalTimeout.arm();
    const value = await Promise.race([
      fn(
        session, boundWaitForReady, ctx.waitForDomStable, boundWaitForNavigation, boundNavigate, ctx.typeInto,
