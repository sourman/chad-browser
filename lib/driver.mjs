...
// `listPageTargets`, `use`, `onEvent`, `captureRequests`, and convenience `evalInPage(expr)`.
async function evalInPage(expression, opts = {}) {
  ...
  if (r.exceptionDetails) {
    // Page-derived text is attacker-controlled (prompt injection). Frame it as
    // untrusted so an agent can't mistake a thrown page string for a system message.
...
// source (reading mid-hydration after a nav). Returns the final URL.
//
...
async function navigate(url, { timeout = 15000, hint = 'navigate' } = {}) {
  ...
  try {
    ...
    // The nav CDP call itself failed — try resetting interception, then fall through
    // to the fresh-target path so we don't wedge forever on a stuck loader.
    await resetInterception();
    navResult = null;
  }
  // An aborted main-frame load (Fetch.failRequest etc.) returns an errorText
  // and never reaches readyState 'complete'. Detect that and recreate the target.
  const aborted = navResult?.errorText || !navResult;
  if (aborted) {
    await resetInterception();
    // Recreate a fresh target — the old one's loader is wedged. Re-enable read
    // domains on it via the attach path.
...
async function handle(conn, line) {
  ...
  try {
    fn = new Function(
      'session', 'waitForReady', 'waitForDomStable', 'navigate', 'typeInto',
      'resetInterception', 'listPageTargets', 'use', 'onEvent', 'captureRequests', 'evalInPage', 'eval',
      wrapped,
    );
  } catch (e) {
  ...
  try {
    const value = await Promise.race([
      fn(
        session, ctx.waitForReady, ctx.waitForDomStable, ctx.navigate, ctx.typeInto,
        ctx.resetInterception, ctx.listPageTargets, ctx.use, ctx.onEvent, ctx.captureRequests,
        ctx.evalInPage, eval,
      ),
      new Promise((_, reject) => {
        timer = setTimeout(
          () => reject(new Error(`eval body timed out after ${bodyTimeoutMs}ms (a pure-sync infinite loop will wedge the driver — run chad-browser down + up to recover)`)),
          bodyTimeoutMs,
        );
      }),
    ]);
