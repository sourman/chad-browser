...
function __describeNodes(val) {
  ...
  function convert(v, depth) {
    ...
    if (typeof v === 'object' && depth < 5 && !(v instanceof Date || v instanceof RegExp || v instanceof Map || v instanceof Set)) {
      var o = {}; for (var k in v) { if (v.hasOwnProperty(k)) o[k] = convert(v[k], depth + 1); } return o;
    }
    return v;
  }
...
async function evalInPage(expression, opts = {}) {
  ...
  const r = await cdp.call('Runtime.evaluate', {
    expression: `${DESCRIBE_NODES_FN}\n__describeNodes((function(){ return (${expr}); })())`,
    returnByValue: true,
...
async function handle(conn, line) {
  ...
  try {
    fn = new Function(
      ...
      'snapshotInteractive', 'eval', 'memory',
      wrapped,
    );
  } catch (e) {
  ...
  try {
    const value = await Promise.race([
      fn(
        ...
        ctx.evalInPage, ctx.snapshotInteractive, eval, req.memory || [],
      ),
